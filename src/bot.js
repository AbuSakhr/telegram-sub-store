...
// ✅ تهيئة قاعدة البيانات (بدل getDb)
(async () => {
await db.init();
})();
...
const reqId = parseInt(data.split('')[2]);
await admin.handleApproveRecharge(ctx, reqId);
}
else if (data.startsWith('reject_recharge')) {
const reqId = parseInt(data.split('')[2]);
await admin.handleRejectRecharge(ctx, reqId);
}
else if (data.startsWith('manage_product')) {
const productId = parseInt(data.split('')[2]);
...
else if (data.startsWith('add_accounts')) {
const productId = parseInt(data.split('_')[2]);
